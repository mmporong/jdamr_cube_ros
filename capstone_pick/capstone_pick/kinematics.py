...
GRASP_BACK = 0.0135     # 큐브 중심에서 접근 방향 **반대로** 이만큼 뒤에 TCP를 둔다
# 25mm 큐브 기준(2026-08-10). 죠 끝(TCP)을 바닥 살짝 위(z=+0.002)에 두는 것이
...
# 위에서 잡으려면 작을수록 유리하다.
#
# 25mm가 물리는 각은 q≈0.15, 그때 파지 중앙은 아랫턱 로컬 x=+0.0054이다.
# 20mm(q≈0.05)는 완전 닫힘(-0.17)과 너무 가까워 죠가 스치듯 지나쳐 닫히는
# 경우가 생겼다(성공률 33%). 25mm는 물림 대역에 여유가 있다.
GRASP_DOWN = 0.0105     # 큐브 중심보다 이만큼 아래
...
APPROACH_BACK = 0.036   # 하강할 때의 back [m] — 큐브 반폭 + 여유


def grasp_q(cx, cy, cz=0.0125, cube_yaw=0.0, up=0.0, pitch=GRASP_PITCH, roll_sign=1.0):
...
def descend_path(cx, cy, cz=0.0125, cube_yaw=0.0, steps=4, insert=3, **kw):
