# Astra S RGB-D P턴 주행 카메라 단독 SLAM 실패 평가
...
- 저텍스처 선회 중 visual odometry 자동 재초기화가 2회 발생했고, 렌더링에서
  분리된 점군 조각이 확인됐다. 따라서 이번 결과는 RGB-D 입력 처리와 실패 복구
  실험의 증거일 뿐, 3D 지도·연속 궤적·절대 위치 정확도 증거로 사용하지 않는다.
...
파일 생성 자체는 성공했지만, 시각 검수에서 공간이 연속적으로 정합되지 않았다.
따라서 위 파일은 디버깅·A/B 비교용으로만 보존하고 지도 이미지나 포트폴리오 대표
미디어로 사용하지 않는다.
