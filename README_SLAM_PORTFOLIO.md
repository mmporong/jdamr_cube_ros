...
최신 검증은 `234 tests, 0 failures, 1 skipped`다. 비교 도구는 서로 다른 source bag,
...
1. 실측 분포를 Gazebo의 LiDAR no-return, scan jitter, IMU bias·noise, wheel slip, TF 지연에
   하나씩 주입한다.
2. 시뮬레이션 ground truth로 Cartographer의 ATE/RPE와 loop-closure 성공·오탐을 측정한다.
3. AMCL과 SLAM Toolbox localization에 위치 점프·가림을 넣어 recovery 시간을 비교한다.
4. distance-only frontier와 gain proxy 정책을 같은 seed·시간 예산으로 비교한다.
5. 카메라 timestamp·intrinsic·extrinsic·이미지 기록이 확보되면 Visual SLAM을 별도 트랙으로
   추가한다.

새 실차 반복 주행은 성공률을 통계로 제시할 때만 필요하다. 다음 단계인 시뮬레이션 강건성
실험과 기존 bag 재분석에는 파이나 로봇을 켤 필요가 없다.
