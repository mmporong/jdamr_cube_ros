# 폐기된 박스 상대 접근 시험 — 과거 기록

> 2026-09-21 사용자 요청으로 박스 전용 접근 정책·실행기·shadow·시작 도구와
> 전용 launch/service/시험 코드를 운영 경로에서 제거했다. 아래 명령과 배포
> 경로는 당시 실패 분석을 위한 기록이며 실행 안내가 아니다. Pi의 접근 서비스는
> 정지·자동 시작 해제 후 재실행을 막는다. 기존 Nav2, 센서 및 지도는 유지한다.
> 기록 보존 기준 커밋은 `b973f9f`이며, 원본 주행 데이터는 삭제하지 않았다.

## 관측 복구 시험 — 2026-09-21 17:00~17:12

사용자 확인 0.74m를 기준으로 후속 시험을 진행했다. 17:00 시도는 약 0.014m
이동 후 `perception_observation_invalid`로 종료했다. 당시 PC의 mDNS/대시보드
접속이 실패하여 순간 원시 관측은 확보하지 못했다. 정지 후 Pi에서 수집한
16초 관측 74개는 전부 정상 검출이었고 수신 시 source age 최대값은 0.471초였다.
이 자료만으로 당시 실패가 미검출인지 입력 지연인지 확정하지 않는다.

제한 시험에만 관측 복구를 추가했다. 후보 미검출 또는 오래된 관측은 즉시
0속도로 전환하고 최대 3초 기다린다. fresh/stable 관측, 새 보호 경로 0속도
응답, 실제 정지, 기존 목표와의 일치가 확인된 뒤 같은 시험 안에서 재개한다.
잘못된 frame/시각/수치, 낮은 confidence와 다른 안전 조건 위반은 종료한다.
전체 30초·0.45m 제한은 대기 중에도 유지하며 목표를 초기화하거나 재무장하지 않는다.

17:11:28에 시작한 온보드 기록 시험 결과:

- 기록 828개, 시작·종료 위치 차이 0.1225m, 추가 시계방향 회전 23.25°.
- `SLOWDOWN` 상태에서도 감속 출력을 유지하며 주행하여 앞선 감속 중단 오류의
  수정 동작을 확인했다.
- 17:11:35.973 `no_box_surface_candidate`로 0속도 대기에 들어갔다.
- 17:11:37.671 재검출 목표가 기존 목표와 달라
  `target_changed_or_odometry_discontinuity`로 종료했다. 최종 명령과 odometry 속도는 0.
- 관측 면 크기가 시작 약 0.289×0.154m에서 종료 약 0.533×0.531m로 달라졌다.
  종료 RGB에서 박스가 화면 왼쪽 경계에 걸려 있다. 다른 면이나 부분 관측이
  목표 추정을 바꿨을 가능성이 있으며, 같은 박스를 끝까지 추적했다고 볼 수 없다.

**주차 검증 미완료.** 관측 재검출 시 무조건 이동하지 않고 목표 불일치에서
종료한 것은 확인했지만, 정밀 주차 성공이나 물리 정확도 검증은 아니다.
다음 수정 대상은 주행 중 박스 면의 동일성 유지와 시야 이탈을 막는 접근 궤적이다.
반복 실차 출발보다 이번 기록으로 목표 변동과 조향 계산을 먼저 대조해야 한다.

기록은 `$HOME/jdamr_artifacts/trial_20260921T171128.jsonl` 및
`$HOME/jdamr_artifacts/trial_20260921T171128_end.jpg`에 보존했다.
Pi 원본은 `$HOME/jdamr_deploy/parking_recovery_record_mXrNE4/`에 있다.
관련 테스트 132개와 독립 코드 리뷰를 통과했으며, 이는 실차 성공 판정과 구분한다.
