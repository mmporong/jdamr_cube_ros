...
def add_low_traction_zone(
        ...
        length_m: float, width_m: float, friction_mu: float) -> None:
    """Add one physical low-friction floor contact patch to an SDF world."""
    ...
    ET.SubElement(model, 'pose').text = f'{center_x_m} 0 0.003 0 0 0'
    link = ET.SubElement(model, 'link', {'name': 'surface'})
    collision = ET.SubElement(link, 'collision', {'name': 'collision'})
    geometry = ET.SubElement(collision, 'geometry')
    box = ET.SubElement(geometry, 'box')
    ET.SubElement(box, 'size').text = f'{length_m} {width_m} 0.006'
    ...
    ET.SubElement(visual_box, 'size').text = f'{length_m} {width_m} 0.006'
...
def prepare(args: argparse.Namespace) -> dict:
    ...
    contract = {
        ...
        'traction_fault': {
            'cause_model': 'physical_contact_friction_patch',
            ...
            'required_transition': [
                'NAVIGATING', 'TRACTION_FAULT', 'PROTECTIVE_STOP',
                'RELOCALIZE', 'LOW_SPEED_RESUME_ONCE_OR_LATCH',
            ],
        },
        'fidelity': {
            'implementation_status': {
                'prepared': [
                    ...
                    'physical low-friction Gazebo contact patch',
                ],
                'pending': [
