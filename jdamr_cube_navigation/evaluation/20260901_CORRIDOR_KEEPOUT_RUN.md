...
> 2026-09-04 정정: 이 문서의 프로세스별 CPU 표는 구버전 `soak_metrics`의 분류 오류로
> recorder가 Collision Monitor에 합산됐을 수 있고 원문 command가 없어 소급 검증할 수
> 없다. 합성 실행에서는 `nav2_container` 자체가 누락되기도 했다. 부하 게이트의 최신
> 판정과 재검증 순서는 `evaluation/20260904_HANDOFF.md`를 기준으로 한다. 아래의
> "현재 수정본"·독립 프로세스·action timeout 설명은 2026-09-01 당시 이력이며 현재
> 운용 구성을 뜻하지 않는다.
